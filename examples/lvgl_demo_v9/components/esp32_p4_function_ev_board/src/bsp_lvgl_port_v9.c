...
#endif
#else
#if CONFIG_LVGL_PORT_ROTATE
    buffer_size = LVGL_PORT_V_RES * LVGL_PORT_BUFFER_HEIGHT * LV_COLOR_DEPTH / 8;
#else
    buffer_size = ALIGN_UP_BY(LVGL_PORT_H_RES * LVGL_PORT_BUFFER_HEIGHT * LV_COLOR_DEPTH / 8, data_cache_line_size);
#endif
    esp_dma_mem_info_t dma_mem_info = {
#if CONFIG_BSP_DISPLAY_LVGL_BUF_INTERNAL
        .extra_heap_caps = MALLOC_CAP_DMA,
#elif CONFIG_BSP_DISPLAY_LVGL_BUF_PSRAM
...
#if CONFIG_LVGL_PORT_ROTATE
    lv_display_t *display = lv_display_create(LVGL_PORT_V_RES, LVGL_PORT_H_RES);
#else
