...
static void audio_feed_task(void *arg)
{
    ...
    while (true) {
        ...
        bsp_extra_pdm_i2s_read((char *)audio_buffer, audio_chunksize * I2S_CHANNEL_NUM * sizeof(int16_t), &bytes_read, portMAX_DELAY);
...
bool app_sr_get_wakeup_result(void)
{
    return detect_flag;
}
...
esp_err_t app_sr_start(bool record_en)
{
    ...
    app_sr_set_language(SR_LANG_EN);
    ESP_GOTO_ON_FALSE(ESP_OK == ret, ESP_FAIL, err, TAG,  "Failed to set language");

    ret_val = xTaskCreatePinnedToCore(&audio_feed_task, "Feed Task", 4 * 1024, (void *)afe_data, 5, &g_sr_data->feed_task, 1);
