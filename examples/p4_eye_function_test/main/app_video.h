 ...
 *
 * Queries device capabilities and sets the desired video format. Configures optional
 * vertical and horizontal flipping if enabled. Closes the device on failure.
 *
 * @param fd File descriptor for the video device.
 * @param init_fmt Desired pixel format for video capture.
 ...
 *
 * Attempts to open the video device at the specified port. Logs an error if
 * the operation fails.
 *
 * @param port The port number for the video device.
 * @return File descriptor of the opened video device, or a negative value on failure.
 */
