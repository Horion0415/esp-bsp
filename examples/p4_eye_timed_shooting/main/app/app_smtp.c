...
#define VALIDATE_MBEDTLS_RETURN(ret, min_valid_ret, max_valid_ret)  \
    do {                                                                        \
        if (ret < min_valid_ret || ret > max_valid_ret) {                       \
            ESP_LOGE                                                  \
            (                                                                       \
                TAG,                                                                \
                "mbedtls error in %s: %d",                                          \
                __FUNCTION__,                                                       \
                ret                                                                 \
            );                                                                      \
        }                                                                       \
...
static const char *TAG = "app_smtp";

static char *buf = NULL;
static unsigned char base64_buffer[128];
static int len;
static size_t base64_len;
...
esp_err_t app_smtp_perform_authentication(void)
{
    ...
    ret = write_ssl_and_get_response(&ssl, (unsigned char *) buf, len);
    VALIDATE_MBEDTLS_RETURN(ret, 300, 399);
