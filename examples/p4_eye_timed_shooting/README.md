...
This demo implements a timed photography feature, allowing users to set a custom timer for taking photos. The captured photos are stored on an SD card and sent to a specified email. Users can also indirectly access the contents of the SD card via the USB HS interface.
...
* A USB-C cable is used to access data on the SD card via USB.
...
Run ``idf.py menuconfig`` and go to ``Example Configuration``: Configurations include setting up the ``email server``, ``email port``, ``sender email``, and ``sender email password`` (this is not the account password but a dedicated SMTP password that needs to be generated). The ``recipient email`` can also be specified.

```
menuconfig > Example Configurration
```

Run ``idf.py menuconfig`` and go to ``Example Connection Configuration``: Configurations include setting up the network connection method and specifying the ``Wi-Fi SSID`` and ``password``.
