...
static void feed_handler(void *arg)
{
    int audio_chunksize = afe_handle->get_feed_chunksize(afe_data);
    int nch = afe_handle->get_channel_num(afe_data);
    size_t samp_len = audio_chunksize;
    ESP_LOGI(TAG, "audio_chunksize:%d\n", audio_chunksize);
    size_t samp_len_bytes = samp_len * I2S_CHANNEL_NUM * sizeof(int32_t);
    ESP_LOGI(TAG, "samp_len_bytes:%d\n", samp_len_bytes);
    //int32_t *i2s_buff = (int32_t *)malloc(samp_len_bytes);
    uint32_t *i2s_buff = (uint32_t *)heap_caps_calloc(1, samp_len_bytes, MALLOC_CAP_SPIRAM);
    assert(i2s_buff);
    size_t bytes_read;
...
static void detect_hander(void)
{
    ...
    while (true)
    {
        ...
        if (detected) {
            ...
            if (mn_state == ESP_MN_STATE_DETECTING) {
                ...
                command = (command_word_t)mn_result->command_id[0];
                //notify();
...
static void speech_before_init()
{
